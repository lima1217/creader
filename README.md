...
- **自动沉淀**: AI 判断有长期价值的阅读知识会无感写入正式 Markdown 页面，翻译、元提示词和普通追问默认不摄入
...
- **自动沉淀**: 跳过普通总结、翻译、闲聊、短期追问和重复解释，减少知识库噪音
- **写入安全**: 后端限制允许目录、清理文件名，并保留 `.reading-memory/ingestion-log.jsonl`
- **设置文案**: Reading Memory 设置改为“自动摄入知识页”，匹配新的直接写入流程

### v0.2.0 (2026-05-17)
- **Reading Memory**: 收紧自动摄入规则，只沉淀高置信阅读知识，避免把每轮对话写入仓库
...
- **Reading Memory**: 新增本地 Markdown 仓库初始化和 AI 回答自动沉淀
...
AI 回答完成后，CReader 会先让 AI 判断这一轮是否值得写入 Reading Memory。只有形成可长期复用的概念、主张、问题、模型、反例或证据链时，才会直接创建或追加到 `books/`、`concepts/`、`questions/`、`claims/` 下的 Markdown 页面，并在 `.reading-memory/ingestion-log.jsonl` 追加写入记录。普通章节总结、翻译、元提示词、苏格拉底式训练交互、闲聊和短答追问默认跳过。后续整理、合并、去重和改链预留给外部 lint agent。
